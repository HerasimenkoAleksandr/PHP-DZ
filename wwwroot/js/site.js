document.addEventListener( 'DOMContentLoaded', () => {
    ...
    const outButton = document.getElementById("out-botton");
	if(outButton) { outButton.onclick = outButtonClick; }
...
function authButtonClick(){
    ...
    const emailInput = document.querySelector('input[name="emailCheck1"]');
    if( ! emailInput) {  throw "'emailCheck' not found bbbbb";    }
    const passwordInput = document.querySelector('input[name="passwordCheck1"]');
    if( ! passwordInput) {  throw "'passwordCheck' not founduuuuu";    }
